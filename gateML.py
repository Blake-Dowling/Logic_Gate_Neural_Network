...
canvas.create_text(((WIDTH_IN_CELLS/2)-3)*CELL_SIZE, 10.25*CELL_SIZE, text = str('ϕ'))
phiLabel = canvas.create_text((0.5)*CELL_SIZE, 11.25*CELL_SIZE, text = "", anchor = "w")
...
def displayState(neural, output):
    ...
    if((time.time()) % 1 < 0.05):
        ...
        ##############################Draw Current Phi Value##############################
        phiText = str(phi)
        canvas.itemconfig(phiLabel, text = phiText)
...
class Params:
    ...
    def __init__(self):
        self.weights = np.array([0.01, 0.01])
...
phi = 1.0
while phi > PHI_TARGET:
    phi = neural.train(neural.INC_AMOUNT)
    if((time.time()) % 1 < 0.1):
